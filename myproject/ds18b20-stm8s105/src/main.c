...
void main(void)
{
  ...
  */
  ...
  while (1)
  {
    ...
    _delay_ms(500);
    uint16_t result = DS18B20_ReadTemperature();
    uint16_t final=result*0.0625;
    uint16_t little=result*62.5;
    uint16_t little1=little%1000;
    printf("temp:%d.%d\r\n",final,little1);

    //printf("temp:%.3f\r\n",DS18B20_ReadTemperaturef());

    //sprintf(ctemp,"%f",DS18B20_ReadTemperaturef());
    //printf("temp:%s\r\n",ctemp);
  }
}
