...
void adc_get_value(void)        //This function runs in main
{
	...
	uint16_t set_temp=0;		//设置的温度值
	uint16_t real_temp=0;	//实际的温度值
	...
	if(adc_en)
	{
		...
		else if(adc_count == 17)
		{
			...
			if(!is_blink_mode())
			{
				real_temp = GetTemp(adc_value_left);
				set_temp=get_setvalue_left();
				if(real_temp >=set_temp)
				{
					set_disp_left(set_temp+getdiff(real_temp-set_temp));
				}
				else
				{
					set_disp_left(set_temp-getdiff(set_temp-real_temp));
				}
			}
		...
		else
		{
			...
			if(!is_blink_mode())
			{
				real_temp = GetTemp(adc_value_right);
				set_temp=get_setvalue_right();
				if(real_temp >=set_temp)
				{
					set_disp_right(set_temp+getdiff(real_temp-set_temp));
				}
				else
				{
					set_disp_right(set_temp-getdiff(set_temp-real_temp));
				}
			}
